...
function hasCollided(object1,object2){
  ...
  if(object1.x+object1.width>object2.x){    
    object1.velocityX = 0;  //Setting the bullet velocity to 0
...
function damage(){
  ...
  fill(0);
  text("Wall damage :"+damage1,windowWidth/2,100); //Displaying the damage
  pop();
...
function mouseClicked(){
  //Creating a text placeholder to edit the speed
  if(mouseX > 50 && mouseX < 90 && mouseY > 360 && mouseY < 390){ 
    if(inp != 0){
      inp.remove();
      button.remove();
    }                                                               
    createObjects("speed",setSpeed,100,440,50);                   
  }     
  //Creating a text placeholder to edit the wieght                  
  else if(mouseX > 200 && mouseX < 270 && mouseY > 360 && mouseY < 390){
    if(inp != 0){
      inp.remove();
      button.remove();
    }                                                                     
    createObjects("weight",setWeight,280,440,50);                        
  }
  //Creating a text placeholder to edit the thickness
  else if(mouseX > 380 && mouseX < 500 && mouseY > 360 && mouseY < 390){
    if(inp != 0){
      inp.remove();
      button.remove();
    }                                                         
    createObjects("thickness",setThickness,480,440,50);       
  }                                                 
}
function createObjects(value,callback,x,y,size){
  ...
  button.mouseClicked(callback);  //Calling the callback function if the mouse is clicked on it
  state = "edit";
  i = value;
}
